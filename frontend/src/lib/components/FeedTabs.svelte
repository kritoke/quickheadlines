<script lang="ts">
	...
	$effect(() => {
		if (listElement) {
			...
			el.addEventListener('scroll', scrollToActiveTab);
			const ro = new ResizeObserver(scrollToActiveTab);
			ro.observe(el);
			return () => {
				el.removeEventListener('scroll', scrollToActiveTab);
				ro.disconnect();
			};
		}
	});
...
>
	<div class="max-w-[1400px] mx-auto">
		<div
			...
			class="sticky top-16 z-20 flex items-center gap-1 px-3 overflow-x-auto scroll-smooth scrollbar-hide rounded-xl shadow-sm bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700"
		>

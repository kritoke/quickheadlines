...
export const themeStyles = ALL_THEMES.map((id) => ({
	id,
	name: id,
	description: `Skeleton ${id} theme`,
}));
...
export function getThemeColors(): {
	...
	return {
		...
		contrast:
			style.getPropertyValue("--color-surface-contrast-light").trim() ||
			"#000000",
		bgSecondary:
			style.getPropertyValue("--color-secondary-100").trim() || "#e2e8f0",
...
export function getCursorColors(): { primary: string; trail: string } {
	const style = getComputedStyle(document.documentElement);
	const secondary =
		style.getPropertyValue("--color-secondary-500").trim() || "#64748b";
	return {
		primary: secondary,
		trail: secondary + "4D",
	};
...
export function getScrollButtonColors(): {
	bg: string;
	text: string;
	hover: string;
} {
	const style = getComputedStyle(document.documentElement);
	const primary =
		style.getPropertyValue("--color-primary-500").trim() || "#334155";
	const primary600 =
		style.getPropertyValue("--color-primary-600").trim() || "#475569";
	return {
		bg: primary,
		text: "#ffffff",
		hover: primary600,
...
export function initTheme() {
	...
	let savedTheme = getStoredValue("quickheadlines-theme", "modern");

	if (LEGACY_THEME_MAP[savedTheme]) {
		savedTheme = LEGACY_THEME_MAP[savedTheme];
		setStoredValue("quickheadlines-theme", savedTheme);
	}
	...
	const effectsEnabled = getStoredBoolean("quickheadlines-effects", true);
	const coolModeEnabled = getStoredBoolean("quickheadlines-coolmode", true);
	themeState.effects = effectsEnabled && coolModeEnabled;
...
export function setTheme(theme: ThemeId) {
	...
	applyTheme(theme);
	setStoredValue("quickheadlines-theme", theme);
}
...
export function toggleEffects() {
	...
	setStoredValue("quickheadlines-effects", String(themeState.effects));
}

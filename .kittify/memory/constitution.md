...
## 5. Task Governance (Double-Entry Rule)
* **Mirroring:** Every Work Package (WP) in Spec Kitty MUST have a corresponding **Epic** in Beads.
* **Granularity:** Every sub-task or implementation step generated by the agent must be created as a **Task** in Beads linked to its parent Epic.
* **Execution:** All Beads commands (`bd`) must be prefixed with `nix develop --command`.
* **Definition of Done:** A task is only "Done" when the Beads issue is closed (`bd close`) AND the Spec Kitty task is checked off.
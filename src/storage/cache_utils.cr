...
module QuickHeadlines::CacheUtils
  ...
  def self.get_cache_dir(config : Config?) : String
  ...
  def self.get_cache_db_path(config : Config?) : String
    File.join(get_cache_dir(config), "feed_cache.db")
  end

  def self.normalize_feed_url(url : String) : String
  ...
  def self.get_db_size(db_path : String) : Int64
  ...
  def self.log_db_size(db_path : String, context : String = "")
  ...
  def self.ensure_cache_dir(cache_dir : String)

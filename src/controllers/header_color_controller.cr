...
class QuickHeadlines::Controllers::HeaderColorController < QuickHeadlines::Controllers::ApiBaseController
  ...
  def save_header_color(request : AHTTP::Request) : QuickHeadlines::DTOs::HeaderColorResponse
    ...
    raise AHK::Exception::ServiceUnavailable.new("Configuration not loaded") if config.nil?
    ...
    raise AHK::Exception::HTTPException.new(413, "Request body too large")
  rescue ex
    Log.for("quickheadlines.http").error(exception: ex) { "Header color save error" }
    raise AHK::Exception::HTTPException.new(500, "Internal server error")
  end

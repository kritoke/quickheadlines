...
**CHANGED:** When a `feed_update` WebSocket message is received, the event SHALL be processed centrally and trigger a debounced, force-refresh of both feed and timeline data. The refresh operations SHALL be awaited before restoring scroll position.

...
### Requirement: Expensive endpoints are rate limited
The system SHALL apply rate limiting to expensive API endpoints.

#### Scenario: Clustering endpoint rate limited
- **WHEN** more than 1 request to /api/cluster is made within 60 seconds
- **THEN** the second request returns 429 Too Many Requests
- **AND** the response includes a Retry-After header

#### Scenario: Admin endpoint rate limited
- **WHEN** more than 1 request to /api/admin is made within 60 seconds
- **THEN** the second request returns 429 Too Many Requests
- **AND** the response includes a Retry-After header